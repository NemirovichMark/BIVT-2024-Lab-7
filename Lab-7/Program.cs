...
namespace Lab_7
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            Purple_5.Research res1 = rep.MakeResearch();

            res1.Add(new string[]{ "123", "231", "32132" });

            res1.Add(new string[] { "dawdaw", "dawdaw", "dadwa" });

            Purple_5.Research res2 = rep.MakeResearch();

            res2.Add(new string[] { "123", "231", "32132" });
            res2.Add(new string[] { "123", null, "32132" });


            (string, double)[] genRep = rep.GetGeneralReport(3);

            for (int i = 0; i < genRep.Length; i++)
            {
                Console.WriteLine(genRep[i]);
            }
        ...
        static void Test_Purple_1()
        {
            ...
            for (int k = 0; k < 7; k++)
            {
                ...
                {
                    int[] marks = new int[scores.GetLength(0) * scores.GetLength(1)];
                    for (int j = 0; j < scores.GetLength(1); j++)
                    {
                        marks[i * scores.GetLength(0) + j] = scores[i, j, k];
                    }
                    judges[k] = new Purple_1.Judge($"Judge {k + 1}", marks);
                }
            }
